...
ENV CGO_ENABLED=0
ENV BUILD_USER=jrparra 
ENV BUILD_HOST=parrajust.in
RUN if [ ! -f syncthing-linux-$TARGETARCH ] ; then \
    go run build.go -version v0.0.1 -no-upgrade build syncthing ; \
...
EXPOSE 42000 22000/tcp 22000/udp 21027/udp
...
HEALTHCHECK --interval=1m --timeout=10s \
  CMD curl -fkLsS -m 2 127.0.0.1:42000/rest/noauth/health | grep -o --color=never OK || exit 1

ENV STGUIADDRESS=0.0.0.0:42000
